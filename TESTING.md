...
| `ExtractorWorkerTests.cs` | `ExtractPhone` and `ExtractFromHtmlAsync` pure logic; malformed payload dead-letters; `Run` routes high-confidence+city to `geocoding-requests`, low-confidence or missing city to `enrichment-requests`; a blob whose download answers 404 completes without extracting |
| `EnrichmentWorkerTests.cs` | Constructor throws when `OpenAIModel` absent; malformed payload dead-letters without calling OpenAI; a full rate gate schedules the same body past the gate's wait and completes without calling OpenAI, and at `MaxGateDeferrals` degrades to the partial data instead; an HTTP 429 schedules a retry past `Retry-After` and completes even at the delivery ceiling, and at `MaxThrottledAttempts` degrades to the partial data instead; every call carries the output-token cap and minimal reasoning effort; any other `ClientResultException` under the retry ceiling abandons for broker redelivery, at/above it degrades to the extractor's partial data and completes; `RetryAfterSeconds` prefers Azure's `retry-after-ms` over `Retry-After`, reads `Retry-After` alone, and falls back without a response; `DeferralDelaySeconds` never schedules before the earliest moment and stops doubling at the cap; `BuildPageContentAsync` (null blob falls back to a placeholder, scripts, styles and markup are dropped and whitespace collapsed, oversized text truncated to the prompt cap); `TryParseEnrichment` truth table (all fields, fallback paths, bool variants) |
