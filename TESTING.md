...
| `LibraryRepositoryTests.cs` | `UpsertEntryAsync`, `GetUnmatchedGameIdsAsync`, `GetGamesForContinuationAsync`, `SetTrophyMatchAsync`, `RefreshTrophyProgressAsync`, and `UpsertDownloadSizesAsync` — a size resolves to a game only through the caller's own `library_entries.title_id`, the largest of two sizes for one platform wins, and a title the caller does not hold writes nothing |
