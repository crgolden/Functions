...
| `TokenCryptoTests.cs` | `Decrypt` reads an unversioned token encrypted by Curator's own Python `TokenCrypto` (a pinned known-answer vector) and a scheme-`0x01` token alike, including the 1-in-256 unversioned token whose first nonce byte is itself `0x01` — the discriminator is the GCM tag, not the byte; it throws `CryptographicException` for a wrong key, tampered ciphertext (versioned or not), or a token shorter than a nonce plus tag. `Encrypt` still writes the unversioned framing, pinned by length, so the write flip cannot ship ahead of Curator's v1-capable decrypt. `EncryptThenDecrypt` round-trips; the constructor rejects a key that doesn't decode to 32 bytes |
